// [INPUT]: 依赖 content::WebUIDataSource、webui::SetupWebUIDataSource（chrome/browser/ui/webui/webui_util.h）、samo/grit/samo_resources*（:resources 生成）、./samo_ui_handler
...
SamoUI::SamoUI(content::WebUI* web_ui) : content::WebUIController(web_ui) {
  ...
  webui::SetupWebUIDataSource(source, base::span(kSamoResources),
                              IDR_SAMO_WEBUI_HTML);
