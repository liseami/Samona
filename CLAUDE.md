...
<product>
架构主张：Samo = 模块 × agent。壳提供窗口、icon navi（浏览器 / 应用 / 应用商店 / 工作区 / 邮件 / 资产，底部账户入口）与侧栏几何；每个模块交出自己的侧栏、面板与面板头部；登录态只有一套，没有 Space/身份。浏览器是第一个模块，也是其余模块的运行时基座（网页视图、登录态、agent 网关）；「应用」维度已落地第一步：桌面（用户主页 + 作品：Local / Private / Public + 热力图）与列表，扫描 localhost 上的应用，点开即在应用维度呈现（不进浏览器、不积累标签）。「工作区」维度：本机目录 = 工作区，面板是目录的 Codex 式对话，agent 在目录里工作的工具是下一步。
