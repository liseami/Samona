...
五张页面共用一套样式与 Tailwind 主题：`index.html` 是应用壳（三层：icon navi + 模块侧栏 + 面板 + 停靠的对话卡，带 preload），`overlay.html` 是命令面板（透明，叠在网页之上），`launcher.html` 是右下角对话入口（透明小视图），`chat.html` 是对话浮窗（跑在子窗口里），`newtab.html` 是新标签页（无 preload 的普通网页）。渲染层从不持有真相：主进程推快照，组件只读镜像；所有动作经 store.send 变成 Command，需要返回值的经 store.query。
...
src/shell/DockResizer.tsx: 停靠对话卡左缘的拖宽热区。
