...
五张页面共用一套样式与 Tailwind 主题：`index.html` 是应用壳（三层：icon navi + 模块侧栏 + 面板 + 停靠的对话卡，带 preload），`overlay.html` 是命令面板（透明，叠在网页之上），`launcher.html` 是右下角对话入口（跑在透明子窗口里），`chat.html` 是对话浮窗（跑在子窗口里），`newtab.html` 是新标签页（无 preload 的普通网页）。渲染层从不持有真相：主进程推快照，组件只读镜像；所有动作经 store.send 变成 Command，需要返回值的经 store.query。
...
src/styles.css: 设计令牌与全局规则（见上）；洗色（border/accent/muted/input）的深浅照搬 Laper 的相对关系；--agent 是 AI 的唯一彩色信号（光标/发光/药丸）；动画令牌 @theme 镜像 shared/motion（--duration-*/--ease-*）；关键帧：bubble-in/thinking/holo/drawer/shimmer、overview-in（标签矩阵）、launcher-in/out 与 panel-in 与 dock-in（对话编舞）、agent-breathe（.agent-glow 边缘发光）、launcher-pulse；层级表：1 卡内热区、2 Popover、3 Tooltip、4 命令面板、5 NavRail 展开层（永远最高），卡片不设 z。
...
src/launcher/: 右下角对话入口（见其 CLAUDE.md）。
