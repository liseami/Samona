 ...
 * [OUTPUT]: 对外提供 useBrowser store（snapshot 镜像 + 壳内一次性请求：rename/identityEditor）、bindBridge()、send()/query()、选择器 selectActiveIdentity/selectActiveTab/selectActiveTabId 与 memo 化的 useIdentityTabs/useFavorites/useIdentityFolders
...
export const useBrowser = create<BrowserState>((set) => ({
  ...
  openIdentityEditor: (identityId) => set({ identityEditor: { value: identityId, nonce: Date.now() } }),
}));
...
export function bindBridge(): () => void {
  const { setSnapshot, requestRename, openIdentityEditor } = useBrowser.getState();
  ...
  const offEvent = window.samo.onEvent((event: ShellEvent) => {
    switch (event.type) {
      ...
      case 'renameFolder':
        ...
        break;
      case 'editIdentity':
        openIdentityEditor(event.identityId);
        break;
      default:
        break;
    }
...
export function useIdentityFolders(): Folder[] {
  const snapshot = useBrowser((s) => s.snapshot);
  return useMemo(() => (snapshot ? snapshot.folders.filter((f) => f.identityId === snapshot.activeIdentityId) : []), [snapshot]);
}
