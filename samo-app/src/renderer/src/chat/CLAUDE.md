...
ChatShell.tsx: 浮层根——外壳恒以浮窗尺寸渲染，snapshot.mode 决定 scale(sx, sy)（药丸尺寸 / 窗口尺寸，锚点右下）；底纹 + 阴影层与内容淡切、Fab 反缩放保形；开 gentle/drawer、关 quick/standard，从停靠回来不播；展开态 ResizeGrips 四边八角热区把屏幕坐标换算成 chat.setBounds；药丸悬停发 chat.hover 让主进程接鼠标。
