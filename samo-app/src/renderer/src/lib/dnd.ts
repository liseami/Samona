 ...
 * [OUTPUT]: 对外提供拖拽 id 编解码（tabDragId/spaceDragId/containerId/parseDndId）、sectionOf(tab)、resolveDrop（把 dnd-kit 的 over 解析为 tab.move 的落点）、sidebarCollision（元素优先于容器的碰撞检测）
...
export const spaceDragId = (id: number): DndId => `space:${id}`;
...
export function parseDndId(raw: DndId): Parsed | null {
  ...
  if (raw.startsWith('space:')) return { kind: 'space', id: Number(raw.slice(6)) };
...
export function sectionOf(tab: Tab): Pick<TabTarget, 'spaceId' | 'pinned' | 'folderId'> {
  return { spaceId: tab.spaceId, pinned: tab.pinned, folderId: tab.folderId };
}

const sameSection = (a: Pick<TabTarget, 'spaceId' | 'pinned' | 'folderId'>, b: Pick<TabTarget, 'spaceId' | 'pinned' | 'folderId'>) =>
  a.spaceId === b.spaceId && a.pinned === b.pinned && a.folderId === b.folderId;

/**
 * 把 dnd-kit 的 over 解析为 tab.move 的落点。
 * 落在元素上 = 插到该元素的位置（同分区内等价于 arrayMove）；落在容器/文件夹头上 = 追加到末尾；落在 Space pip 上 = 移到该 Space 的散装区末尾。
 */
export function resolveDrop(active: Tab, over: Parsed, tabs: Tab[], activeSpaceId: number): TabTarget | null {
  ...
  switch (over.kind) {
    ...
    case 'favorites':
      return { spaceId: null, pinned: true, folderId: null, index: END };
    case 'pinned':
      return { spaceId: activeSpaceId, pinned: true, folderId: null, index: END };
    case 'loose':
      return { spaceId: activeSpaceId, pinned: false, folderId: null, index: END };
    case 'folder':
    case 'folderHead':
      return { spaceId: activeSpaceId, pinned: false, folderId: over.id, index: END };
    case 'space':
      if (over.id === active.spaceId) return null;
      return { spaceId: over.id, pinned: false, folderId: null, index: END };
    default:
      return null;
  }
}
