/**
 * [INPUT]: 依赖 ../../lib/dnd 的 parseDndId，../../store/browser 的 useBrowser，./Favicon，@shared/model 的 tabTitle
 * [OUTPUT]: 对外提供 DragGhost 组件：DragOverlay 里跟随指针的浮起卡片（标签行 / Space pip）
...
export function DragGhost({ dndId }: { dndId: string }) {
  ...
  if (parsed.kind === 'space') {
    const space = snapshot.spaces.find((s) => s.id === parsed.id);
    return space ? <div className="flex h-7 w-7 items-center justify-center rounded-lg border border-border bg-card text-sm shadow-md">{space.emoji}</div> : null;
  }
  if (parsed.kind === 'tab') {
