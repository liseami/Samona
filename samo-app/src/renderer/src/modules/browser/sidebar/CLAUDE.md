...
浏览器模块的 Arc 级侧栏。自上而下（头部、宽度与拖拽调宽属于壳；地址与导航在面板头部）：顶部留白 pt-2 → agent 控制条 → 收藏网格 → 固定网格 → 文件夹与散装标签（分隔线上悬停 Clear）→ agent 分组（每个任务空间一组：Bot + 任务名 + 脉冲点 + Take control / Hand back + 它的标签）→ 底栏（下载 + DevTools）；右缘拖拽调宽。
...
SidebarFooter.tsx: 底栏——下载浮层 + DevTools。
