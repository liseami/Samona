 ...
 * [OUTPUT]: 对外提供 UrlField 组件：面板头部居中的地址栏——只做「看」与「进入」：显示当前标签的短地址（https 带锁；悬停链接时淡色顶替显示目标地址），点击打开 ⌘T 同款命令面板（有地址时 editUrl 模式并带入全地址，否则 newTab）
...
export function UrlField() {
  ...
  const hover = useBrowser((s) => s.snapshot?.hoverUrl ?? null);
  const url = tab?.url ?? '';
  const shown = displayUrl(url);
  const hovered = hover ? displayUrl(hover) || hover : '';
  ...
  return (
    ...
    >
      ...
      {/* 悬停链接：Chromium 显示在左下角，Samo 在这里以淡色顶替当前地址 */}
      <span className={cn('min-w-0 flex-1 truncate text-center text-base', hovered ? 'text-muted-foreground' : shown ? 'text-foreground' : 'text-muted-foreground')}>{hovered || shown || 'Search or enter URL'}</span>
