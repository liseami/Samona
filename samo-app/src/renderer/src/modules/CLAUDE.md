...
## 成员清单
registry.tsx: ModuleDef 与 MODULE_REGISTRY——browser、apps、appstore、workspace、assets 接入真实实现，mail 接 placeholder，design 接陈列页；新模块 = 新目录 + 这里一行。
...
appstore/: 应用商店维度——别人发布的应用陈列，右侧 Add 收进自己的应用；mock 数据（见其 CLAUDE.md）。
