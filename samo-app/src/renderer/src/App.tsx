 ...
 * [OUTPUT]: 对外提供 App 根组件：Laper ProjectEditorShell 结构——页面底 bg-sidebar，一行 gap-2 pt-2 pb-2 pl-0 pr-2：NavRail（与底同色）| 侧栏卡（SoftPanel：Header h-12 + 模块侧栏）| 面板卡（SoftPanel，浏览器模块时网页视图内缩 1px 叠在其上）| 停靠时的对话卡（第四张 SoftPanel）；折叠态侧栏卡消失，控制条横在面板卡之上；写入 html.dark 与 --identity
      ...
      <div className="drag flex h-full w-full gap-2 bg-sidebar pt-2 pb-2 pl-0 pr-2">
        ...
        {chat?.mode === 'docked' && (
          <div className="relative h-full min-h-0 shrink-0" style={{ width: chat.dockWidth }}>
            ...
            <div className={`h-full overflow-hidden ${SOFT_PANEL}`}>
