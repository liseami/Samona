 ...
 * [OUTPUT]: 对外提供 App 根组件：Laper ProjectEditorShell 结构——页面底 bg-sidebar，一行 gap-2 pt-2 pb-2 pl-0 pr-2：NavRail（与底同色）| 侧栏卡（SoftPanel：Header h-10 + 模块侧栏）| 面板卡（SoftPanel：模块的 PanelHeader h-10 + 面板体；浏览器模块时网页视图从头部下方内缩 1px 叠在其上）| 停靠时的对话卡（第四张 SoftPanel）；折叠态侧栏卡消失，控制条横在面板卡之上；可见身份被 agent 驱动时面板卡 agent-glow；写入 html.dark 与 --identity
...
export default function App() {
  ...
  // 可见身份被 agent 驱动：面板卡边缘发光（网页之上还有 agent 光标层）
  const agentLive = useBrowser((s) => {
    const id = s.snapshot?.identities.find((i) => i.id === s.snapshot?.activeIdentityId);
    return !!id && id.ownership === 'agent' && id.agentState !== null && s.snapshot?.layout.module === 'browser';
  });
  useEffect(() => bindChat(), []);
