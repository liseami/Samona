...
palette-window.ts: PaletteWindow——壳浮层的透明子窗口（子窗口不受主窗口可拖拽区影响，也是唯一真正压在网页之上还能交互的层）：承载 ⌘T 命令面板（openPalette）与用户菜单（openUserMenu，OverlayEvent 联合），打开时铺满主窗口内容区并聚焦，首次打开等页面 did-finish-load 再投递（否则事件丢失、面板不出现），背景幕点击/Esc 即关，关闭即隐藏、把焦点还给主窗口并向壳发 overlayClosed。
