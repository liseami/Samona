...
让外部 AI agent 驱动 Samo 的网关。ego-browser-v2（ego lite 开源的 harness）以为自己在和一个支持 flatten 会话的 Chromium 对话，并且宿主提供 `globalThis.ego`；这里就是那个宿主的服务端：传输（gateway）→ 语义（session）→ 协议适配（cdp-bridge）→ 页面之眼（snapshot）。设计取自 phi 的 AgentCDP（指针文件发现、会话凭证、ownership、captureWindow）并补上 phi 缺的服务端按 Space 过滤 target。

数据流：CLI `{cdp}` 帧 → session.handleCdp → CdpBridge：无 sessionId 的 Target.*/Browser.* 在本地仿真（getTargets 只列选中 Space 的标签），带 sessionId 的命令路由到该标签的 webContents.debugger；debugger 事件回推为 `{event:'cdp'}`。
...
task-spaces.ts: Space ↔ task space 线形换算、EGO_CODE 错误码表、egoError（resolved 形）与 EgoRejection（reject 形）。
