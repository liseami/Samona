...
「应用」维度的主进程侧：Samo 的产品主张是把用户 vibe coding 出来的工作台收进侧栏，这里是第一步——把正在 localhost 上跑的应用找出来、投影成 store 里的 apps/activeAppId。网页本身仍由浏览器引擎承载：一个应用 = 一个带 appId 的应用视图——不落盘、不进浏览器侧栏、不改浏览器的活动标签，只在应用维度由引擎呈现（每个维度呈现自己的标签），扫描发现应用不在跑就关掉它；登录态、历史、agent 网关全部照用。默认呈现桌面（activeAppId = null）。云端（Samo 部署）列表预留。
