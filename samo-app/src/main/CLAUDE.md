...
## 成员清单
index.ts: 引导。app.setName('Samo') 固定 userData 路径；nativeTheme → store.dark；history/downloads 装配到标签页 session；dev 时从 ELECTRON_RENDERER_URL 取壳与新标签页地址，prod 走 file://；单实例锁；恢复落盘状态或 seed；退出前 flush 落盘并关闭网关。
