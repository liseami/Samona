...
## 成员清单
store.ts: ChatStore——线程（可绑定 workspaceId）/消息/流式/形态/未读/停靠宽度/回答者能力（needsKey、model）；append/appendDelta/finish/remove/appendTool/updateTool/stopStreaming/ensureWorkspaceThread；toPersisted/hydrate（chat.json v1，工具字段可选）。
...
prompt.ts: buildSystemPrompt——Samo AI 的宪法：身份、ego-browser 1.2.5 运行时地图、工作区目录（有则告知）（page/locator/browser/taskSpaces/fetch/cdp）、正确性法则、task space（本线程固定名，默认不 complete，用户可在侧栏看）、控制权交接（用户接管即硬停）、当前浏览器上下文。
