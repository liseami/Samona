...
## AI 对话
真相在主进程 chat/（线程、消息与工具胶囊、流式、形态）。三形态：closed（右下角 Laper 药丸，透明子窗口）、floating（可拖出应用、可缩放的子窗口）、docked（面板卡右侧的第四张卡），切换由编舞器做成连续的窗口几何动画（曲线/时长取 shared/motion）。回答者是 ChatProvider 插槽：配了 Anthropic 密钥（对话面板的接入卡或 ANTHROPIC_API_KEY）就是 AgentProvider——Claude 用唯一的 `browser` 工具写 ego-browser 脚本，经 samo-browser 子进程与网关真正驱动浏览器，agent 在自己的身份里工作、侧栏可围观；没配就是引导语。⌘I 开关。
...
- **renderer**（React）：只读镜像 + 命令出口。三层壳（icon navi + 模块侧栏 + 面板卡：头部 导航·地址·工具 + 面板体）是壳视图，红绿灯自绘；网页是主进程叠在面板头部之下的独立视图（圆角容器上溢藏到头部之下，网页上直下圆；非浏览器模块或标签矩阵时隐藏）；最上层还有透明的 overlay 视图承载 ⌘T 命令面板；壳之下有用户看不见的「后台视图」，供 agent 在用户看别处时继续工作。
...
src/renderer/ - 壳页 index.html（shell/ + modules/ + chat/ 停靠卡）、命令面板 overlay.html、对话入口 launcher.html（子窗口）、对话浮窗 chat.html（子窗口）、新标签页 newtab.html（共用 styles.css；icons/ 为 Laper 的 Pika 图标库；components/effects 为 WebGL 效果）
