...
- **renderer**（React）：只读镜像 + 命令出口。侧栏是壳视图，网页是主进程叠在壳之上的独立视图（壳先画好同几何的面板与边线）；最上层还有透明的 overlay 视图承载 ⌘T 命令面板；壳之下有用户看不见的「后台视图」，供 agent 在用户看别处时继续工作。
...
src/renderer/ - 壳页 index.html、命令面板 overlay.html、新标签页 newtab.html（共用 styles.css；icons/ 为 Laper 的 Pika 图标库）
